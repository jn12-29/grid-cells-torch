...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Explicit train split .npz path. When omitted, directory mode writes train.npz under data/datasets/<dataset-id>/.",
    )
    ...
    parser.add_argument(
        "--eval_output",
        default=None,
        help="Explicit eval split .npz path. Requires --output; omitted in directory mode only when --train_only is set.",
    )
    parser.add_argument(
        "--train_only",
        action="store_true",
    ...
    register_config_overrides(
        parser,
        sections=("task", "visualization", "data_generation"),
    )
    return parser.parse_args()


def _resolve_visualization_bins(cfg) -> dict:
    """Resolve visualisation bin counts from config and safe defaults."""
...
def _resolve_animation_settings(cfg) -> dict:
    """Resolve shared animation settings from config and safe defaults."""
    vis_cfg = getattr(cfg, "visualization", None)
    defaults = {
        "anim_num_traj": 4,
        "anim_fps": 20,
...
def main() -> None:
    ...
    explicit_train_path = args.output
    explicit_eval_path = args.eval_output
    ...
    num_samples = getattr(data_generation_cfg, "num_samples", None)
    ...
    if eval_num_samples is None and not args.train_only:
        eval_num_samples = cfg.training.eval_num_samples
    ...
    if explicit_file_mode:
        ...
        if output_path is None and eval_output_path is None:
            raise ValueError(
                "No output path provided. Set --output or use directory mode."
            )
    else:
    ...
    animation_settings = _resolve_animation_settings(cfg)
    pc_ensembles, hdc_ensembles = get_cell_ensembles(cfg)
    validate_cell_code_contract(cfg, pc_ensembles, hdc_ensembles)

    vis_path = None
    if args.visualize and output_path is not None:
        if explicit_file_mode:
            vis_path = getattr(data_generation_cfg, "vis_output", None)
            if vis_path is None:
    ...
    anim_path = None
    if args.animate and output_path is not None:
        if explicit_file_mode:
            anim_path = getattr(data_generation_cfg, "anim_output", None)
    ...
    if args.visualize or args.visualize_progress:
        visualization_bins = _resolve_visualization_bins(cfg)
