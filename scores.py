...
class GridScorer(object):
  ...
  def calculate_sac(self, seq1):
    ...
    std_seq1 = np.power(np.subtract(np.divide(sum_seq1_sq, n_bins),
        (np.divide(np.square(sum_seq1), n_bins_sq))), 0.5)
    std_seq2 = np.power(np.subtract(np.divide(sum_seq2_sq, n_bins),
        (np.divide(np.square(sum_seq2), n_bins_sq))), 0.5)
    covar = np.subtract(np.divide(seq1_x_seq2, n_bins),
  ...
  def plot_sac(self, sac, mask_params=None, ax=None, title=None, *args, **kwargs):
    ...
    if mask_params is not None:
      ...
      ax.add_artist(plt.Circle((center, center), mask_params[0] * self._nbins, fill=False, edgecolor='k'))
      ax.add_artist(plt.Circle((center, center), mask_params[1] * self._nbins, fill=False, edgecolor='k'))
    ax.axis('off')
    if title is not None:
      ax.set_title(title)
