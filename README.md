...
- `task.pc_target_family` is PC-only target encoding; keep DoS/DoG there instead of adding them to shared `task.targets_type`, which also configures HDC targets and loss mode. Use `task.decode_type=top_k` plus `task.decode_top_k` when you want No-Free-Lunch-style center voting instead of weighted-mean position decoding for DoS/DoG targets.
