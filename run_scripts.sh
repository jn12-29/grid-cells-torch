...
11. Train with config overrides
CUDA_VISIBLE_DEVICES=0 python train.py --training.epochs 20 --training.eval_every 1 --visualization.anim_step 2

12. Train with the recommended scale-invariant analytic decoding contract made explicit
...
13. Train with emphasized timestep-0 place-cell target loss
CUDA_VISIBLE_DEVICES=0 python train.py --training.first_pos_loss_multiplier 50

14. Train with lighter statistical analysis settings and population scale histogram/GMM plots
...
15. Train with only per-unit Banino-style grid geometry analysis
